...
class ServerAdapter(BaseRollout):
    ...
    async def update_weights(self, weights: Generator[tuple[str, torch.Tensor], None, None], **kwargs):
        ...
        # Remove stale IPC socket file before binding (ZMQ close() doesn't
        # delete it, and previous runs or calls may have left it behind)
        ...
        # clean up
        s.close()
        # Remove IPC socket file — ZMQ close() doesn't delete it, causing
        # "Address already in use" on the next update_weights call.
        ipc_path = self.zmq_handle.replace("ipc://", "")
        if os.path.exists(ipc_path):
            os.unlink(ipc_path)
        del buffer
