...
class FusionAgentLoopDeprecated(AgentLoopBase):
    ...
    This agent loop uses SandboxFusion SDK with command replay for statefulness.
    The new fusion_agent_loop uses a centralized session server with true statefulness.
    ...
    - Statefulness via command history replay (slower than true statefulness)
    ...
    def __init__(self, *args, **kwargs):
        import warnings
        warnings.warn(
            "FusionAgentLoopDeprecated is deprecated. Use 'fusion_agent_loop' instead, "
            "which uses SweRex session server for true statefulness.",
            DeprecationWarning,
            stacklevel=2
        )
